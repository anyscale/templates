...
- **Dependencies pinned.** Declare template deps in `templates/<name>/requirements.txt`, the BYOD `Dockerfile` (`RUN pip install "pkg==x.y.z"`), or notebook `!pip install` — with exact versions. Most templates also ship a compiled `python_depset.lock`; the repo-root `dependencies/` holds its inputs — the image freezes and `template.depsets.yaml` — and `references/dependencies.md` owns that system.
...
| `check-dep-delivery` | depset config, lock installed, no bare pip, `==` pins | `check-dep-delivery` | `python3 scripts/hooks/check-dep-delivery.py` |
