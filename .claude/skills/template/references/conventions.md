...
- **Dependencies pinned via a depset.** A template that adds Python deps beyond the base image carries the **depset footprint**: `templates/<name>/requirements.txt` (pinned) + an entry in `dependencies/template.depsets.yaml` + a compiled `python_depset.lock`. Authoring, flags, and the compile command: `depsets.md`. BYOD templates pin in the `Dockerfile` instead. The three depset files must stay consistent — but a stale lock is caught only by the `check-depsets` CI job, not locally (`depsets.md` "Enforcement").
