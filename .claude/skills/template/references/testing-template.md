...
- **A Ray Data write defaults to `SaveMode.APPEND` — pass `mode=SaveMode.OVERWRITE`.** `write_parquet`/`write_csv`/`write_json` name their part files uniquely, so a second run *adds* to the directory instead of replacing it. Anything that then reads the whole directory back silently grows: `fintech_fraud_risk` passed ten daily probes and then OOM'd its driver on the eleventh, concatenating every run's output at once. `/mnt/cluster_storage` hides this in CI (fresh cluster per run) while still hitting any workspace user who re-runs the notebook; `/mnt/shared_storage` and artifact storage survive everything. A unique per-run path works too — `tempfile.mkdtemp()`, as `ray-data-batch-image-classification` does.
