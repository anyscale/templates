...
if [[ ! -x ./scripts/depsets/update_deps.sh ]]; then
  failures+=("depset toolchain: ./scripts/depsets/update_deps.sh missing or not executable (wrong CWD, or repo not checked out?)")
else
