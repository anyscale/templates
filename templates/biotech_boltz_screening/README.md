...
**How to read the timings:** Ray Data is lazy. The `map_batches()` calls in `src/pipeline.py` only build a plan — the entire pipeline executes at the terminal `write_parquet()`. Wrapping a stopwatch around each `map_batches()` call would report ~0s for every stage and dump the whole run's cost on the write, so the pipeline reports one true wall-clock time and then prints Ray Data's own `ds.stats()` for the per-stage breakdown, which the executor records as each operator actually runs. Two things to expect in that report: operators are not 1:1 with the five stages, because Ray Data fuses neighbours that share a compute strategy (post-processing and the write become one operator, while the GPU actor-pool stage stays on its own); and the per-operator times overlap rather than partition the run, because the stages stream concurrently. `ds.stats()` is the idiomatic way to profile any Ray Data pipeline.
...
The final cell reprints the pipeline's summary metrics — measured wall time, throughput, and cost estimate — and shows how to promote this exact notebook logic to a scheduled **Anyscale Job** with a single CLI command. Same code, same cluster config -- no rewrite needed. The per-operator breakdown is the `ds.stats()` report printed by Step 4.
