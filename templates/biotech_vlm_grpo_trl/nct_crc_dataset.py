"""
Preprocess NCT-CRC-HE (colorectal H&E patches, 9 tissue classes) into the parquet
that train_grpo_trl.py and make_sft_data.py read.
...
Row schema: `prompt` (chat messages; the image rides along inside the user turn as a
base64 JPEG data URI), `env_class`, `reward_spec.ground_truth` (class name),
`extra_info`. train_grpo_trl.py::to_trl_rows() turns this into TRL's row format.
    ...
    def process_fn(example, idx):
        ...
        return {
            "prompt": [
                ...
                # prompt reaches the model as literal JSON text. Target schema:
