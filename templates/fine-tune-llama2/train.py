...
def training_function(kwargs: dict):
    ...
    _test_tokenizer(config["model_name"])
    tokenizer = get_tokenizer(model_name=config["model_name"], special_tokens=special_tokens)
    ...
    num_steps_per_epoch = math.ceil(train_ds_len / config["batch_size_per_device"])
    ...
    for epoch in range(num_epochs):
        ...
        with tempfile.TemporaryDirectory(dir=config["output_dir"]) as temp_checkpoint_dir:
        ...
        if perplex < config["stop_perplexity"]:
            stop_perplexity=config["stop_perplexity"]
            print(f"Perplexity reached {perplex} < {stop_perplexity}. Stopping.")
            break
...
def parse_args():
    ...
    parser.add_argument(
        "--ctx-len", type=int, default=512, help="Context length to use."
    )
...
def main():
    ...
    config.update(
        **{
            ...
            "stop_perplexity": args.stop_perplexity,
    ...
    # Set the local checkpoint directory which may get used when the remote cloud storage bucket checkpointing is disabled.
    os.environ["RAY_AIR_LOCAL_CACHE_DIR"] = config["output_dir"]

    # Set up runtime_env for Ray Train
    ray.init(
        runtime_env={
            "env_vars": {
    ...
    # Read data
    train_ds = ray.data.read_json(config["train_path"])
    if args.test_path is not None:
        valid_ds = ray.data.read_json(config["test_path"])
    else:
        valid_ds = None

    # Read special tokens json file
    with open(config["special_token_path"], "r") as json_file:
        special_tokens = json.load(json_file)["tokens"]

    # Specify the cloud bucket path to use when saving training artifacts
    # ANYSCALE_ARTIFACT_STORAGE is the default cloud storage bucket created for Anyscale users
