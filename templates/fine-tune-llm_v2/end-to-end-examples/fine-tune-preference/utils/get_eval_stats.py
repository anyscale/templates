...
def eval_rows(row):
    if row["summary_generation_raw_model_output"] is None or row["qa_generation_answers"] is None or row["judge_mc_answers"] is None or "No Judge Output" in row["judge_mc_answers"]:
        return []
...
lens = np.array([len(merged_results.iloc[i]["text"].split()) for i in range(len(merged_results))])

wins = [compare(*vals) for vals in zip(merged_results["accuracy_x"], merged_results["num_words_x"], merged_results["accuracy_y"], merged_results["num_words_y"])]
losses = [compare(*vals) for vals in zip(merged_results["accuracy_y"], merged_results["num_words_y"], merged_results["accuracy_x"], merged_results["num_words_x"])]
