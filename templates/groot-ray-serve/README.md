...
def run_live_rollout(policy_url, worker_dir):
    ...
    output_dir = "/tmp/live_rollout_output"
    os.makedirs(output_dir, exist_ok=True)
    results_file = f"/tmp/live_rollout_{int(time.time())}.json"
    ...
    cmd = (
        ...
        f"--episodes 1 --max-steps 8 --action-horizon 8 "
...
# Update this path if sim_worker.py lives elsewhere on the cluster.
...
# print("  Isaac Lab boot + rollout typically takes 30-90s on a warm worker")
# print()

# # Poll the future in a thread so we can print progress on the main thread
# result_holder = {}
# def _collect():
...
# print(f"\nSim task finished in ~{elapsed}s. Exit code: {result['exit_code']}")
# print()

# if result["gif_path"] and os.path.exists(result["gif_path"]):
#     print(f"Displaying live-generated GIF: {result['gif_path']}")
#     display(Image(filename=result["gif_path"]))
# else:
#     print("Live rollout did not produce a GIF in time. Falling back to pre-recorded.")
#     if result["stderr_tail"]:
#         print(f"(stderr tail: {result['stderr_tail']})")
#     fallback = "g1_groot_n17_zeroshot.gif"
#     if os.path.exists(fallback):
#         display(Image(filename=fallback))
#     else:
#         print(f"(fallback GIF not found at {fallback})")
```
