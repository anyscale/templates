...
class TorchRecWrapper:
    def __init__(self, config: RecsysConfig):
        ...
        self.config = config
        # Global seeding for parity with dlrm_main
        ...
        # Separate streams for training and validation
        self._train_epoch_idx = 0
        self._current_step = 0
        self._last_checkpoint_time = time.time()  # Track last checkpoint time for time-based checkpointing

        self._train_pipeline = TrainPipelineSparseDist(self.model, self.optimizer, ray.train.torch.get_device())
        self._val_pipeline = TrainPipelineSparseDist(self.model, self.optimizer, ray.train.torch.get_device())
    ...
    def get_optimizer(self) -> torch.optim.Optimizer:
        ...
        dense_optimizer = KeyedOptimizerWrapper(
            ...
            lambda params: torch.optim.Adagrad(params, lr=self.config.learning_rate, eps=self.config.optimizer_eps),
        )
    ...
    @classmethod
    def from_checkpoint(cls, config: RecsysConfig, checkpoint_dir: str) -> 'TorchRecRunner':
        """Create a TorchRecRunner instance and load from checkpoint."""
        runner = cls(config)
    ...
    def _validate(self) -> Dict[str, float]:
        ...
        # Compute local sample-weighted validation loss (no cross-rank reduce)
        validation_loss = (total_loss_sum / float(max(1, total_sample_count))) if total_sample_count > 0 else 0.0
        # Compute local AUROC (no cross-rank aggregation)
        auroc_result = auroc_metric.compute().item()
...
def train_loop(config_dict: dict):
    ...
    # If checkpoint is not None, means we are resuming from worker failure
    if checkpoint:
        ...
        with checkpoint.as_directory() as temp_checkpoint_dir:
            logger.info(f"Starting from checkpoint locally: {temp_checkpoint_dir}")
            wrapper = TorchRecWrapper.from_checkpoint(recsys_config, temp_checkpoint_dir)
    else:
        wrapper = TorchRecWrapper(recsys_config)

    for epoch in range(recsys_config.num_epochs):
        wrapper.run()

        # Save checkpoint every N epochs
        if wrapper._should_save_checkpoint_at_epoch(epoch):
            wrapper._save_checkpoint(epoch, wrapper._current_step)

    # Save final checkpoint if enabled
    if recsys_config.save_checkpoint_at_end:
        wrapper._save_checkpoint(wrapper._train_epoch_idx - 1, wrapper._current_step, is_final=True)
