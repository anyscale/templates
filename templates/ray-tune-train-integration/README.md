...
tuner = tune.Tuner(
    ...
    tune_config=TuneConfig(
        ...
        max_concurrent_trials=2  # Run 2 trials at once (adjust based on cluster size)
    ),
    run_config=RunConfig(
...
asha_scheduler = ASHAScheduler(
    ...
    grace_period=2,  # Let all trials run at least 2 epochs
...
try:
    ...
    bayesopt_search = BayesOptSearch(
        metric="accuracy",
        mode="max",
        random_search_steps=4  # Start with 4 random trials for initialization
    )
