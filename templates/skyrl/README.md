...
```bash
uv run --isolated examples/train/gsm8k/gsm8k_dataset.py --output_dir  /mnt/cluster_storage/data/gsm8k
```

This script converts the Huggingface GSM8K dataset to two Parquet files with the [schema required by SkyRL](https://docs.skyrl.ai/docs/datasets/dataset-preparation).
...
```bash
SKYRL_RAY_PG_TIMEOUT_IN_S=90 bash examples/train/gsm8k/run_gsm8k.sh \
...
<img src="https://raw.githubusercontent.com/anyscale/templates/main/templates/skyrl/assets/gsm8k_wandb.png" width=1500px />


## Creating a new environment or task

Now that you've run a basic example to teach an LLM to solve math word problems, you might want to start training on your own custom task! Check out the SkyRL docs for [creating a new environment or task](https://docs.skyrl.ai/docs/tutorials/new_env) for a full walkthrough of the simple steps to implement a custom multi-turn environment using the SkyRL-Gym interface. The commands needed to run the multi-turn example in the linked tutorial on Anyscale are shown below.
...
### Training your model
```bash
SKYRL_RAY_PG_TIMEOUT_IN_S=90 bash examples/train/multiply/run_multiply.sh \
...
- Explore more advanced algorithms, like [PPO](https://github.com/NovaSky-AI/SkyRL/tree/main/examples/train/ppo) or [DAPO](https://docs.skyrl.ai/docs/algorithms/dapo)
- Explore more advanced tasks like [agentic search (Search-R1)](https://skyrl.readthedocs.io/en/latest/examples/search.html) or [code generation](https://docs.skyrl.ai/docs/harbor).
- Optimize your training pipeline using [Fully Async Training](https://docs.skyrl.ai/docs/tutorials/fully_async).
