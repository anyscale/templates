...
def list_objects():
    ...
    for obj_state in ray.util.state.list_objects(raise_on_missing_output=False):
        if obj_state.object_size > 1000:
            print(
                ...
                f", CALL SITE: {obj_state.call_site.split('/')[-1]}"
            )
    print("-" * 100)
...
if __name__ == "__main__":
    ...
    print("Inspecting the impact of a Ray generator task on the object store")
    for obj_ref in generate_blocks.remote():
        transform_block.remote(obj_ref)

    print("Inspecting the impact of a Ray non-generator task on the object store")
